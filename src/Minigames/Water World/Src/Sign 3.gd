	...
	if onArea == true:
		if Input.is_action_just_pressed("select"):
			...
			match Global.language:
				"English":
					ChangeDialogue(dialogues.sign3Talk["english"])
					ArrowInvisible(dialogues.sign3Talk["english"])
				"Portuguese":
					ArrowInvisible(dialogues.sign3Talk["portuguese"])
					ChangeDialogue(dialogues.sign3Talk["portuguese"])
	else:
